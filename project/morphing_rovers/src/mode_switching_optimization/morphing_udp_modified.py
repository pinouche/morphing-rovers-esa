...
class Rover:
    def __init__(self, chromosome) -> None:
        ...
        # build training data
        self.current_mode_velocity = None
        self.best_mode_velocity = None
        self.training_data = []
    ...
    def update_rover_state_stateless(self, rover_view, mode_view, angle, position, distance_vector,
                                     mode_efficiency, cooldown,
                                     onehot_representation_of_mode, current_mode, original_distance, latent_state):
        ...
        angle_diff = minimal_angle_diff(angle_to_sample, angle)

        rover_state = torch.Tensor([mode_efficiency, cooldown / MODE_COOLDOWN, angle_diff / np.pi, \
                                    float(distance_to_sample),
                                    angle / np.pi / 2] + onehot_representation_of_mode)

        switching_mode, angular_change, latent_state = self.Control(rover_view, rover_state, latent_state)
        ...
        # get mask of current mode
        form = self.form_masks[current_mode]
        # determine velocity factor for current form and terrain
        velocity_factor = velocity_function(form, mode_view)
        mode_efficiency = velocity_factor

        # Logic for switching the mode
        # If morphing is not on cooldown and the neural network initiates a morph
        if (cooldown == 0) and (switching_mode > 0):
        ...
        onehot_representation_of_mode = list(np.eye(1, NUMBER_OF_MODES, current_mode)[0])

        # update position and orientation of the rover
        direction = torch.Tensor([np.cos(angle), np.sin(angle)])
        position = position + MAX_DV * velocity_factor * direction
        angle = angle + MAX_DA * angular_velocity_factor

        return angle, position, mode_efficiency, cooldown, onehot_representation_of_mode, \
            current_mode, latent_state, velocity_factor, current_mode
    ...
    def update_rover_state(self, rover_view, mode_view, distance_vector, original_distance):
        ...
        self.training_data.append(([rover_view.numpy(force=True), rover_state.numpy(force=True), self.latent_state.numpy(force=True)]))
        ...
        # update position and orientation of the rover
        self.position = self.position + MAX_DV * velocity_factor * self.direction
...
class morphing_rover_UDP:
    ...
    def fitness(self, rover, num_steps_to_run):
        """
        ...
        for heightmap in range(MAPS_PER_EVALUATION):
            for scenario in range(SCENARIOS_PER_MAP):
                self.run_single_scenario(heightmap, scenario, num_steps_to_run)
    ...
    def run_single_scenario(self, map_number, scenario_number, num_steps_to_run):
        print(f"COMPUTING FOR SCENARIO {scenario_number}")
        ...
        for timestep in range(0, num_steps_to_run):
            ...
            boolean = True
            if boolean:
                best_mode = self.rover.get_best_mode(mode_view)
                current_mode = copy.deepcopy(self.rover.current_mode)

                dict_mode = {"current_mode": 0, "best_mode": 0}
                for m_string in ["current_mode", "best_mode"]:
                    if m_string == "current_mode":
                        m = current_mode
                    else:
                        m = best_mode
                    # update rover state 8 times for 2 different modes
                    for i in range(1, MODE_COOLDOWN + 1):
                        if i == 1:
                            position = self.rover.position.clone()
                            dist_vector = distance_vector.clone()
                            angle = copy.deepcopy(self.rover.angle)
                            mode_efficiency = self.rover.mode_efficiency.clone()
                            cooldown = copy.deepcopy(self.rover.cooldown)
                            onehot_representation_of_mode = list(np.eye(1, NUMBER_OF_MODES, m)[0])
                            latent_state = self.rover.latent_state.clone()

                        r_view, m_view = self.env.extract_local_view(position, angle, map_number)

                        angle, position, mode_efficiency, cooldown, onehot_representation_of_mode, \
                            current_mode, latent_state, velocity, m = self.rover.update_rover_state_stateless(r_view,
                                                                                                           m_view, angle,
                                                                                                           position,
                                                                                                           dist_vector,
                                                                                                           mode_efficiency,
                                                                                                           cooldown,
                                                                                                           onehot_representation_of_mode,
                                                                                                           m,
                                                                                                           original_distance,
                                                                                                           latent_state)

                        dist_vector = sample_position - position
